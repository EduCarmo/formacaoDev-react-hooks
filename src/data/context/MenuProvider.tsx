...
export function MenuProvider(props: any) {
  ...
  function selecionarItem(url: string) {
    let novasSecoes = secoes.map((secao: { itens: any[]; }) => {
  ...
  function alterarSecao(secaoSelecionada: MenuSecao) {
    let novasSecoes = secoes.map((secao: { titulo: string; aberta: any; }) => {
  ...
  const ctx = { mini, toggleMini, secoes, alterarSecao };
  return (
